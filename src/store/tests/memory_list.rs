...
/// A `repo`/`kind`-only filter — the pre-`ListFilter` call shape.
fn filt<'a>(repo: Option<&'a str>, kind: Option<&'a str>) -> ListFilter<'a> {
    ListFilter {
        repo,
        kind,
        ..ListFilter::default()
...
fn lists_all_in_stable_created_desc_id_asc_order() {
    ...
    let page =
        memory_list::list_memories(&conn, &filt(None, None), 0, 0, SortBy::Created).expect("list");
...
fn limit_and_offset_window_the_middle_page() {
    let (_dir, conn) = seeded_db();
    let page =
        memory_list::list_memories(&conn, &filt(None, None), 3, 2, SortBy::Created).expect("list");
...
fn offset_past_end_returns_empty_with_full_total() {
    let (_dir, conn) = seeded_db();
    let page = memory_list::list_memories(&conn, &filt(None, None), 5, 100, SortBy::Created)
        .expect("list");
    assert_eq!(page.total, 8);
...
fn repo_filter_restricts_total_and_rows() {
    let (_dir, conn) = seeded_db();
    let page = memory_list::list_memories(&conn, &filt(Some("beta"), None), 0, 0, SortBy::Created)
        .expect("list");
...
fn kind_filter_restricts_total_and_rows() {
    let (_dir, conn) = seeded_db();
    let page =
        memory_list::list_memories(&conn, &filt(None, Some("decision")), 0, 0, SortBy::Created)
            .expect("list");
    assert_eq!(page.total, 4);
    assert_eq!(
        ids(&page.rows),
...
fn slug_is_file_stem_id_dash_slug() {
    let (_dir, conn) = seeded_db();
    let page =
        memory_list::list_memories(&conn, &filt(None, None), 1, 0, SortBy::Created).expect("list");
    let row = &page.rows[0];
