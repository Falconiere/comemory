...
fn shipped_copy() -> (tempfile::TempDir, PathBuf) {
    ...
    for name in ["_journal.json", "0020_candidate_observations.snapshot.json"] {
...
fn schema_drift_registry_matches_shipped_snapshot() {
    ...
    assert_eq!(
        ...
        "run_generate wrote {written:?}: the registry differs from \
         migrations/0020_candidate_observations.snapshot.json (a struct changed without `just migration <name>`, \
