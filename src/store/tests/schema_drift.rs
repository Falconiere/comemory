...
fn shipped_copy() -> (tempfile::TempDir, PathBuf) {
    ...
    for name in ["_journal.json", "0022_replica_journal.snapshot.json"] {
...
fn schema_drift_registry_matches_shipped_snapshot() {
    ...
    assert_eq!(
        ...
        "run_generate wrote {written:?}: the registry differs from \
         migrations/0022_replica_journal.snapshot.json (a struct changed without `just migration <name>`, \
