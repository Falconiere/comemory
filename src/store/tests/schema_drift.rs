...
fn shipped_copy() -> (tempfile::TempDir, PathBuf) {
    ...
    for name in ["_journal.json", "0023_memory_write_intent.snapshot.json"] {
...
fn schema_drift_registry_matches_shipped_snapshot() {
    ...
    assert_eq!(
        ...
        "run_generate wrote {written:?}: the registry differs from \
         migrations/0023_memory_write_intent.snapshot.json (a struct changed without `just migration <name>`, \
