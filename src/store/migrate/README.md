...
| `historical_document_revision.rs` | `repair` | v28 post-pass: rebuild the historical staged `remote_document` layout into the released accepted-revision layout, preserving accepted rows and removing incomplete rows and their search entries in one transaction |
