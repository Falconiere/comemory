...
| `vector.rs` | `MemoryHit` | `vec0` insert and KNN against `memory_vec` / `code_vec`, plus `is_loaded` — whether `sqlite-vec` registered on a connection, behind `comemory doctor`'s check |
