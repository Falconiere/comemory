...
| `memory_intent.rs` | `record` | `memory_write_intent` row CRUD — the one outstanding "a memory write is in flight" row per memory, recorded before the markdown moves and cleared in the transaction that finishes the write |
...
| `memory_repository.rs` | `label` | Stored repository label for a live or soft-deleted memory, used before managed sync uploads |
...
| `replica_cursor.rs` | `Cursor`, `load`, `save`, `all` | One applied position per workspace, stored with the stream epoch it is valid under, so a restored upstream reads as a replaced stream rather than as agreement |
| `replica_outbox.rs` | `PendingOperation`, `enqueue`, `pending`, `settle`, `record_attempt` | The durable outbox of local mutations a peer has not accepted: enqueued in the mutation's own transaction, drained oldest first, settled with the upstream's answer, and kept after a refusal as evidence |
| `replica_journal.rs` | `NewOperation`, `append`, `redact_payload`, `stream_epoch` | The `replica-v1` write unit: store the immutable payload, append the ordered feed position, update the entity's revision — all in the caller's transaction, so a position can never name a payload that is not there. Also the erasure that blanks payload bytes while keeping the digest as the resurrection barrier |
| `replica_read.rs` | `FeedRow`, `page`, `head`, `revision`, `live_digests` | `replica-v1` reads: the ordered page above a cursor joined to the payload each position *named* (never today's bytes), the head, one entity's revision/tombstone state, and the live digest set the manifest buckets |
| `replica_receipt.rs` | `Receipt`, `record`, `lookup` | What this engine answered for one operation, written in the accept transaction and read back on a replay — why a lost acknowledgement costs a round trip instead of a second effect |
| `replica_staging.rs` | `put_part`, `state`, `assemble`, `discard` | Parts of an oversized revision: a re-sent part replaces its earlier copy, assembly refuses while a declared part is missing, and nothing is visible to `changes` or `manifest` until activation |
...
| `seed_scan.rs` | `live_memories_after` | The ordered live-memory id scan journal seeding resumes from — ordered by id, because two memories can share a `created_at` and none share an id |
