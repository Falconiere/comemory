...
| `edges.rs` | `insert` | `edges` table CRUD: typed upserts, weighted accumulation, outgoing neighbors, the `supersedes_chain` recursive walk, delete-by-node, the code-graph/`memory_rank` weighted-edge queries, `co_changed`/`imports` scoped deletes, `count_by_rel` (behind `maintenance::overview`'s edge totals), and the `file_neighbor_rows` one-hop query; every `domains::graph` algorithm calls this rather than owning its own SQL |
