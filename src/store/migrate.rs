...
/// v20: the three candidate-observation tables — the per-query envelope, its
...
/// 0023 SQL (the memory write-intent and needs-embedding tables). Public so
/// tests can replay historical schema states exactly as an old binary created
/// them.
pub const M_V23: &str = include_str!("../../migrations/0023_memory_write_intent.sql");
/// 0024 SQL (code generations and the pulled-projection tables). Public so
/// tests can replay historical schema states exactly as an old binary created
/// them.
...
/// 0026 SQL (the locally readable repository approval map). Public so tests
/// can replay historical schema states exactly as an old binary created them.
pub const M_V26: &str = include_str!("../../migrations/0026_repository_approval.sql");
