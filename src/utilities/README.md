...
| `shared_text.rs` | `for_share`, `label_for_share`, `Shared` | The free-text policy for anything shared with another machine (#254): absolute machine paths become `<path>`, then a secret-rule match withholds the field — a query entirely, a caller label by dropping it. Never logs the value |
