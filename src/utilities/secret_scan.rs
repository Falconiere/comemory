//! The curated secret scan: what may not leave this machine.
//!
//! Shared rather than owned by one capability. Sync pushes memory bodies
//! through it and the documents capability scans extracted text before a
//! revision is journalled, and neither may depend on the other — so the rule
//! set lives here, where both already reach.
//!
...
//!
//! `src/domains/capture/rules.toml` is a SECOND, deliberately different rule
//! set — same rule ids, tighter word-anchored patterns — that the capture
//! path loads itself. The two files are not a copy that drifted, so do not
//! collapse them without deciding which patterns capture should run.
