//! The execution context every command core runs against.
//!
//! [`Ctx`] bundles [`Paths`] + [`Config`] with a connection that is either
//! borrowed from the caller or opened lazily on first use, so conn-free
//! commands (`doctor`, `rebuild`, `ast`, `install-hooks`, `completions`) never
//! touch the database and a job worker gets exactly one connection of its own.
//!
//! Transport-neutral on purpose (#166): `cli`, `serve`, and every command core
//! construct the same `Ctx`, so neither delivery adapter owns it.
