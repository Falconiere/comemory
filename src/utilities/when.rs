//! Parsing for the temporal flag values (`--since`, `--until`, `--as-of`).
//!
...
//!
//! Parsing an instant is transport-neutral, so it lives here. Turning three
//! parsed instants into a retrieval window is not: that is
//! [`scope::scope_from_flags`](crate::domains::retrieval::scope::scope_from_flags),
//! which owns `TimeScope` and moved there with #178 so no shared module
