...
//!
//! [`run`] hands the whole synchronous call — session lock included — to
...
//! closure, so a refused write never opens the connection nor runs a line of
...
/// Run a core against a fresh connection. An [`Access::Write`] on a
...
/// Lock the session, open the live database, build a [`Ctx`] and call `f`.
