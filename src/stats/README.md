...
feedback counters, per-symbol code feedback counters, and the raw retrieval
log. The persisted `retrieval_log.source` / `feedback_events` vocabularies and
the `q-<yyyymmdd>-<8hex>` query-id contract moved to `utilities::telemetry` and
`utilities::query_id` with #166, so retrieval no longer depends on the learning
capability for them.
...
chunk-to-parent identity resolution rule and every transaction boundary
(`StatsDb::conn_mut().transaction()`), and calls the store helpers from inside
it.
...
| `feedback.rs` | `Source` | Per-memory feedback counters: `used` and `irrelevant`, plus `Source` (the routes' `explicit\|implicit` request words with `parse` and the one `provenance()` mapping onto the stored value). Owns the transaction; the SQL is `store::feedback` and the provenance vocabulary is `utilities::telemetry` |
