//! `maintenance::doctor::system::run` — `GET /api/v1/doctor/system` (console-api
//! spec §8): the **facts** half of doctor. Where `maintenance::doctor::run` probes
...
/// `(path, bytes)`. Shares `maintenance::doctor::checks::newest_matching` with the
