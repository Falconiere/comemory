# domains/maintenance/doctor/

**What belongs here:** the individual `comemory doctor` health probes and
the pass that runs all of them — split out of `src/domains/maintenance/doctor.rs` once the
...
forward-compat `SchemaTooNew` fallback, which stay in
`src/domains/maintenance/doctor.rs`.
...
`src/domains/maintenance/doctor.rs` (`pub mod checks;`) and callers import
concrete paths.
