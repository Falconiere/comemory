//! `maintenance::prune::{Request, run}` — the shared middle of `comemory prune` /
...
use crate::domains::maintenance::retention::{low_value, stale_code};
    ...
    for id in low_value_ids {
        match delete::soft_delete(paths, conn, id, None, None) {
            ...
            Err(Error::NotFound(_)) => {
                ...
                derived_stale |= delete::mirror_soft_delete(conn, id, None)?.0;
            }
            Err(e) => return Err(e),
        }
    }
