...
//! Document-preservation mirror for
//! `src/domains/maintenance/rebuild/copy.rs`. Exercised directly through an
//! `ATTACH`ed real "old" database via the public
...
//!
//! NOT CURRENTLY COMPILED: no `#[path]`/`mod` declares this file, so
//! `cargo nextest list` shows none of its cases. It was orphaned by an
//! earlier refactor that removed its donor `rebuild/documents.rs`; #176
//! moved it with its folder and repointed its paths without wiring it back
//! in, because restoring the cases is a coverage change rather than a move.
//! Tracked by [#178](https://github.com/Falconiere/comemory/issues/178).
...
use crate::domains::maintenance::rebuild::copy::copy_preserved_tables_from_old;
