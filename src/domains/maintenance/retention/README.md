...
(`cli::prune`, `cli::gc`, calling `domains::memories::MemoryStore::delete`), or
`maintenance::prune`'s own confirmed apply; nothing in `retention/` writes to
the store.
...
`src/domains/maintenance/retention.rs` (`pub mod <name>;`) and callers import
concrete paths. The owned `Report` is a sibling of this folder at
`../retention_report.rs`, because both delivery surfaces and the command core
share it.
