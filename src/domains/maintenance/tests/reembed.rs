...
//! Mirror test for `src/domains/maintenance/reembed.rs` — `POST /api/v1/doctor/reembed`
...
fn run_fails_with_embedder_when_the_first_row_cannot_be_embedded() {
    ...
    let cfg = Config::defaults();
    let mut ctx = Ctx::lazy(&paths, &cfg);
    let err = maintenance::reembed::run(
        &mut ctx,
        maintenance::reembed::Request {
            target: maintenance::reembed::Target::Memories,
            batch: None,
...
fn run_over_an_empty_corpus_is_a_no_op() {
    ...
    let resp = maintenance::reembed::run(
        &mut ctx,
        maintenance::reembed::Request::default(),
        &cmd,
        None,
    )
    .expect("empty reembed run");

    assert_eq!(resp, maintenance::reembed::Response::default());
}
...
fn a_both_run_embeds_the_leg_the_width_fits_and_reports_the_other_skipped() {
    ...
    let resp = maintenance::reembed::run(
        &mut ctx,
        maintenance::reembed::Request {
            target: maintenance::reembed::Target::Both,
            batch: None,
        },
        &cmd,
        Some(&sink),
...
fn an_explicit_code_leg_the_width_does_not_fit_is_refused_before_any_write() {
    ...
    let err = maintenance::reembed::run(
        &mut ctx,
        maintenance::reembed::Request {
            target: maintenance::reembed::Target::Code,
            batch: None,
        },
        &cmd,
        None,
