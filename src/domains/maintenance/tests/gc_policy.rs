...
//! Mirror test for `src/domains/maintenance/gc_policy.rs` — `GET|PUT /api/v1/gc/policy`
...
//! whose mtime is pushed two days back so `maintenance::gc::run` under the patched
...
fn get_reports_the_configured_windows_without_creating_the_db() {
    ...
    let policy = maintenance::gc_policy::get(&mut ctx).expect("gc policy get");
...
fn update_persists_both_windows_and_leaves_other_keys_alone() {
    ...
    let cfg = Config::defaults();
    let mut ctx = Ctx::lazy(&paths, &cfg);
    let policy = maintenance::gc_policy::update(
        &mut ctx,
        maintenance::gc_policy::UpdateRequest {
...
fn update_rejects_a_zero_window_and_leaves_the_file_untouched() {
    ...
    let cfg = Config::defaults();
    let mut ctx = Ctx::lazy(&paths, &cfg);
    let err = maintenance::gc_policy::update(
        &mut ctx,
        maintenance::gc_policy::UpdateRequest {
...
fn update_to_one_day_makes_gc_reap_a_two_day_old_trashed_file() {
    ...
    let resp =
        maintenance::gc::run(&mut ctx, maintenance::gc::Request {}).expect("gc under defaults");
    ...
    let resp = maintenance::gc::run(&mut ctx, maintenance::gc::Request {})
        .expect("gc under the new window");
