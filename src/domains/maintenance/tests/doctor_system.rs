...
//! Mirror test for `src/domains/maintenance/doctor/system.rs` — `GET /api/v1/doctor/system`
...
fn run_reports_the_current_schema_on_a_real_store() {
    ...
    let system = maintenance::doctor::system::run(&mut ctx, None).expect("system run");
