...
//! Mirror test for `src/domains/learning/tune.rs`. Seeds a real corpus + golden file via
//! the binary, then calls `tune::run` directly against a `Ctx` opened
...
use comemory::domains::learning::tune;
...
fn run_without_apply_reports_and_writes_nothing() {
    ...
    let resp = tune::run(&mut ctx, req).expect("tune run");
...
fn run_is_deterministic_for_a_pinned_seed() {
    ...
    let run_once = |paths: &Paths, cfg: &Config, golden: &std::path::Path| {
        ...
        tune::run(&mut ctx, req).expect("tune run")
    };
