...
use comemory::domains::learning::bandit;
...
fn run_without_apply_reports_and_never_writes_config() {
    ...
    let req = bandit::Request {
        golden: Some(golden.to_string_lossy().into_owned()),
        golden_only: true,
        k: 3,
        apply: false,
    };
    let report = bandit::run(&mut ctx, req).expect("bandit run");
...
fn run_still_upserts_bandit_arms_when_apply_is_false() {
    ...
    bandit::run(&mut ctx, req).expect("bandit run");
...
fn run_apply_refused_when_bandit_disabled_in_config() {
    ...
    let err = bandit::run(&mut ctx, req).expect_err("apply must be refused");
    assert!(
