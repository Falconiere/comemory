...
use comemory::domains::learning::{learning_proposals, tune};
...
fn list_offers_only_open_runs_whose_knobs_actually_differ() {
    ...
    let proposals = learning_proposals::list(&mut ctx).expect("list");
...
fn apply_writes_config_toml_stamps_the_row_and_retires_the_proposal() {
    ...
    let cfg = Config::defaults();
    let mut ctx = Ctx::borrowed(&paths, &cfg, &mut conn);
    let applied = learning_proposals::apply(&mut ctx, "prop1").expect("apply");
    ...
    let mut ctx = Ctx::borrowed(&paths, &cfg, &mut conn);
    assert!(
        learning_proposals::list(&mut ctx).expect("list").is_empty(),
...
fn discard_retires_a_proposal_without_touching_config_toml() {
    ...
    let cfg = Config::defaults();
    let mut ctx = Ctx::borrowed(&paths, &cfg, &mut conn);
    let discarded = learning_proposals::discard(&mut ctx, "prop2").expect("discard");
    ...
    let mut ctx = Ctx::borrowed(&paths, &cfg, &mut conn);
    assert!(learning_proposals::list(&mut ctx).expect("list").is_empty());
}
...
fn apply_refuses_an_unknown_id_a_second_apply_and_a_discarded_run() {
    ...
    let err = learning_proposals::apply(&mut ctx, "nosuchrun").expect_err("unknown id");
    assert!(matches!(err, Error::NotFound(_)), "got {err:?}");

    learning_proposals::apply(&mut ctx, "once").expect("first apply");
    let err = learning_proposals::apply(&mut ctx, "once").expect_err("second apply");
    assert!(matches!(err, Error::BadRequest(_)), "got {err:?}");

    let err = learning_proposals::apply(&mut ctx, "dropped").expect_err("discarded run");
    assert!(matches!(err, Error::BadRequest(_)), "got {err:?}");

    let err = learning_proposals::discard(&mut ctx, "once").expect_err("applied run");
    assert!(matches!(err, Error::BadRequest(_)), "got {err:?}");
}
...
fn a_run_whose_knobs_are_not_a_full_knob_set_is_skipped_not_fatal() {
    ...
    let err = learning_proposals::apply(&mut ctx, "legacy").expect_err("apply legacy");
    assert!(
