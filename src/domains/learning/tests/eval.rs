...
use comemory::domains::learning::eval;
...
fn run_scores_the_real_pipeline_against_the_golden_file() {
    ...
    let report = eval::run(&mut ctx, req).expect("eval run");
...
fn running_eval_twice_writes_two_rows_newest_first() {
    ...
    eval::run(&mut ctx, req()).expect("first eval run");
    eval::run(&mut ctx, req()).expect("second eval run");

    let history_req = eval::Request {
        history: true,
        ..req()
    };
    let rows = eval::history(&mut ctx, &history_req).expect("history read");
...
fn history_on_an_empty_table_returns_an_empty_vec_not_an_error() {
    ...
    let rows = eval::history(&mut ctx, &history_req).expect("history read on empty table");
...
fn history_limit_caps_the_row_count() {
    ...
    for _ in 0..3 {
        ...
        eval::run(&mut ctx, req).expect("eval run");
    }
    ...
    let rows = eval::history(&mut ctx, &history_req).expect("history");
...
fn missing_golden_and_no_feedback_is_unavailable() {
    ...
    let err = eval::run(&mut ctx, req).expect_err("no golden pairs at all");
    assert!(
