...
use comemory::domains::learning::{console, feedback};
...
fn every_read_answers_empty_on_a_data_dir_with_no_database() {
    ...
    let summary = console::summary(&mut ctx).expect("summary");
    ...
    assert!(console::evals(&mut ctx, 20).expect("evals").is_empty());
    let page = console::expansions(&mut ctx, 20, 0).expect("expansions");
    assert!(page.items.is_empty());
    assert_eq!(page.total, Some(0));
    let golden = console::golden_set(&mut ctx, None).expect("golden set");
...
fn evals_derives_delta_is_baseline_and_is_best_from_real_rows() {
    ...
    let rows = console::evals(&mut ctx, 20).expect("evals");
...
fn summary_counts_feedback_and_its_implicit_share_from_real_verdicts() {
    ...
    let verdict = |source: Option<&str>| feedback::Request {
    ...
    feedback::run(&mut ctx, verdict(None)).expect("record feedback");
    ...
    let implicit = feedback::run(&mut ctx, verdict(Some("implicit"))).expect("implicit");
    ...
    let explicit = feedback::run(&mut ctx, verdict(Some("explicit"))).expect("explicit");
...
fn golden_set_merges_the_real_harvest_with_a_file() {
    ...
    let harvest_only = console::golden_set(&mut ctx, None).expect("golden set");
    ...
    let merged = console::golden_set(&mut ctx, file.to_str()).expect("golden set");
...
fn expansions_pages_mined_rows_strongest_support_first() {
    ...
    let first = console::expansions(&mut ctx, 2, 0).expect("expansions");
    ...
    let second = console::expansions(&mut ctx, 2, 2).expect("expansions page 2");
    assert_eq!(second.items.len(), 1);
    assert_eq!(second.items[0].from, "index");
    assert!(!second.has_more);

    let all = console::expansions(&mut ctx, 0, 0).expect("expansions, limit 0");
