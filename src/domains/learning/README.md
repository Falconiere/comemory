...
| `feedback_share.rs` | `journal`, `Caller` | Record-time journalling of a shareable verdict (#254), inside the verdict's own transaction — the only moment a code rowid is known to mean the symbol the payload names. Decides what may be shared (provenance, approved repository), records the caller's surface and actor, and never enqueues on the outbox |
...
| `replica_payload.rs` | `FeedbackEventV1`, `Target` | One shared verdict on the `replica-v1` wire: event id, device, original time, provenance, surface, actor, the namespaced `origin_query_id`, and the canonical memory or code target (with the code version when known); plus the `ev-<32 hex>` event-id shape both event kinds use. `auto_coactivation` is deliberately not shareable |
