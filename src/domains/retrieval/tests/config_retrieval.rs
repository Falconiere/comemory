...
//! Mirror test for `src/domains/retrieval/config_retrieval.rs`: the knob projection with
...
fn empty_update() -> retrieval::config_retrieval::UpdateRequest {
    retrieval::config_retrieval::UpdateRequest::default()
}

#[test]
fn get_projects_the_live_knobs_and_declares_every_range() {
    let cfg = Config::defaults();
    let knobs = retrieval::config_retrieval::get(&cfg);
...
fn update_persists_only_the_supplied_keys_and_returns_the_new_knobs() {
    ...
    let knobs = retrieval::config_retrieval::update(
        &mut ctx,
        retrieval::config_retrieval::UpdateRequest {
            rrf_k: Some(30.0),
            graph_hops: Some(1),
...
fn an_out_of_range_knob_is_a_bad_request_and_leaves_the_file_byte_identical() {
    ...
    let cfg = Config::defaults();
    let mut ctx = Ctx::lazy(&paths, &cfg);
    let err = retrieval::config_retrieval::update(
        &mut ctx,
        retrieval::config_retrieval::UpdateRequest {
            rrf_k: Some(0.0),
            ..empty_update()
...
fn every_validated_bound_is_reachable_through_the_update_path() {
    ...
    for (label, req) in [
        ...
        let mut ctx = Ctx::lazy(&paths, &cfg);
        let Err(err) = retrieval::config_retrieval::update(&mut ctx, req) else {
