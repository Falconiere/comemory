...
//! `retrieval::suggest::run` against a real store (console-api spec §3): real
//! `query_expansions` rows and real `retrieval_log` rows written by real
//! tracked searches through `retrieval::find` / `retrieval::search_code` — never a
...
use comemory::retrieval::{find, search_code, suggest};
