...
//! [`crate::domains::retrieval::code_prior`] (PageRank, activation, working-set
...
use crate::domains::retrieval::code_prior::{self, CodePriorParts, Signals};
use crate::domains::retrieval::code_ref_collect::{self, RawRef};
...
///
/// Owns `query` so `retrieval::context::run` can return it independently of the
...
pub fn assemble(
    ...
    // Seeded with every resolved ref's file; `file_neighbors` dedups, so a
    // memory citing three symbols in one file still seeds that file once.
    let seed_files: Vec<(&str, &str)> = code_refs
        .iter()
        .map(|c| (c.repo.as_str(), c.path.as_str()))
        .collect();
    let neighbors = neighbors::file_neighbors(conn, &seed_files, DEFAULT_MIN_WEIGHT)?;
    Ok(Bundle {
