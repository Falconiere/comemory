...
//! `memories::update::run` against a real store: memories written through the
//! real `memories::save::run`, a real migrated SQLite database, and the markdown
...
use comemory::domains::memories::save::Verbatim;
use comemory::domains::memories::{self, Frontmatter, Kind, Ref, References};
...
fn ac6_tags_patch_keeps_the_id_and_rewrites_the_markdown() {
    ...
    let mut ctx = Ctx::borrowed(&paths, &cfg, &mut conn);
    let shown =
        memories::show::run(&mut ctx, memories::show::Request { id: saved.id }).expect("show");
...
fn ac6_body_patch_mints_a_new_id_that_supersedes_the_old() {
    ...
    let old = {
        ...
        memories::save::run(&mut ctx, req, false, None).expect("save")
    };
    ...
    // `retrieval::rerank` reads (`memories::show` reports it on the OLD id).
    let old_shown = {
        let mut ctx = Ctx::borrowed(&paths, &cfg, &mut conn);
        memories::show::run(&mut ctx, memories::show::Request { id: old.id.clone() })
            .expect("show old")
    };
    ...
    let mut ctx = Ctx::borrowed(&paths, &cfg, &mut conn);
    let new_shown =
        memories::show::run(&mut ctx, memories::show::Request { id: resp.id }).expect("show new");
...
fn an_empty_patch_is_a_no_op_that_still_succeeds() {
    ...
    let old = {
        let mut ctx = Ctx::borrowed(&paths, &cfg, &mut conn);
        memories::save::run(&mut ctx, save_request("nothing to patch here"), false, None)
            .expect("save")
    };
...
fn kind_and_repo_patches_land_in_the_frontmatter() {
    ...
    let mut ctx = Ctx::borrowed(&paths, &cfg, &mut conn);
    let shown =
        memories::show::run(&mut ctx, memories::show::Request { id: old.id }).expect("show");
...
fn quality_out_of_range_is_bad_request() {
    ...
    let old = {
        let mut ctx = Ctx::borrowed(&paths, &cfg, &mut conn);
        memories::save::run(&mut ctx, save_request("quality range guard"), false, None)
            .expect("save")
    };

    let mut ctx = Ctx::borrowed(&paths, &cfg, &mut conn);
    let err = memories::update::run(
        &mut ctx,
        &old.id,
        memories::update::Request {
            quality: Some(9),
...
fn unknown_id_is_not_found() {
    ...
    let err = memories::update::run(&mut ctx, "deadbeef", memories::update::Request::default())
...
fn soft_deleted_id_is_not_found() {
    ...
    let old = {
        let mut ctx = Ctx::borrowed(&paths, &cfg, &mut conn);
        memories::save::run(&mut ctx, save_request("about to be deleted"), false, None)
            .expect("save")
    };
    {
        let mut ctx = Ctx::borrowed(&paths, &cfg, &mut conn);
        memories::delete::run(&mut ctx, &old.id).expect("delete");
    }
