...
fn a_blob_oid_that_is_not_a_git_object_id_is_refused() {
    use comemory::domains::sync::exchange::code_import_rules::validate;
...
fn the_quota_refuses_a_batch_that_would_overflow_the_repo() {
    use comemory::domains::sync::exchange::code_import_rules::{SYMBOL_QUOTA, validate};
    let req = CodeImportRequest {
        repo: "r".into(),
        head: None,
        mined_commit: None,
        files: vec![comemory::domains::sync::exchange::CodeFileWire {
            path: "a.rs".into(),
            blob_oid: "a".repeat(40),
            symbols: vec![comemory::domains::sync::exchange::CodeSymbolWire {
                symbol: "f".into(),
                kind: "function".into(),
                lang: "rust".into(),
                line_start: 1,
