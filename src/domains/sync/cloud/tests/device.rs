...
use comemory::domains::sync::cloud::CLIENT_ID;
use comemory::domains::sync::cloud::device::DeviceCodeResponse;
...
fn mint_org_key_returns_the_org_scope_over_a_real_socket() {
    ...
    assert_eq!(
        creds.version,
        comemory::domains::sync::auth_file::AUTH_SCHEMA_VERSION
    );
...
fn unscoped_mint_is_refused_rather_than_persisted() {
    ...
    for config in [
        ...
        let mut progress = Vec::new();
        let err = comemory::domains::sync::cloud::login(&server.base, &mut progress)
...
fn org_status_reports_a_revoked_key_instead_of_raising() {
    ...
    // "not authenticated".
    let err = comemory::domains::sync::cloud::org_status(&server.base, "cmk_wrong", None)
