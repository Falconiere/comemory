//! `domains::code::repo_admin` — `POST /api/v1/repos`, `PATCH /api/v1/repos/{name}`,
...
pub struct ConnectRequest {
    ...
    /// the same rule `domains::code::reindex_policy::repo_context` uses.
    #[serde(default)]
...
/// [`crate::domains::code::git_utils::repo_label`] derives for the root — its main
...
/// The default repo label: [`crate::domains::code::git_utils::repo_label`] for the
/// repository at `root`, else `root`'s last path component.
fn default_label(root: &str) -> Result<String> {
    if let Some(label) = crate::domains::code::git_utils::repo_label_at(Path::new(root)) {
        return Ok(label);
    }
    Path::new(root)
        .file_name()
