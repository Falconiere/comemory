...
/// root at one would leave `utilities::repo_root` resolving file ids against a
...
/// repository of its own — [`crate::domains::code::index_code`] uses this to refuse to
...
/// here is benign — resolving a repo's remote is advisory (`domains::code::repos`'s
...
/// minting a `<worktree-dir>` repo label. [`crate::domains::code::install_hooks`]
