...
| `hooked_refresh.rs` | `RefreshStats`, `Checkout` | The local half of `comemory sync --action auto`: index the checkout a git hook fired in under its main-worktree label, then re-index every other registered repo whose hooks are comemory's and whose HEAD moved — from any cwd, inside the sync pass lock |
| `hooks.rs` | `Request` | Shared middle of `comemory hooks` / `GET\|POST /api/v1/hooks` — per-hook read and toggle over the two state stores: the four reindex hooks (`post-commit`, `post-merge`, `post-checkout`, `post-rewrite`) on disk under the Git common directory, and the config-backed search→edit reinforcement row |
