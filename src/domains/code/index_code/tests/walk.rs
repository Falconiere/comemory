...
//! Mirror test for `src/domains/code/index_code/walk.rs`. Every function in that
//! file is `pub(crate)` (shared with `cli::index_code`'s `--extract`
//! path), so it is exercised indirectly through the public
//! `crate::domains::code::index_code::run` entry point rather than called directly —
...
/// and `src/domains/code/ast/tests/chunk.rs` so every consumer chunks the same corpus.
