...
**What belongs here:** the walk-path internals of
`domains::code::index_code` — the
...
`Response` shape, which stay in `src/domains/code/index_code.rs`, and
symbol extraction itself, which is `domains::code::ast::extract`'s job.
...
`src/domains/code/index_code.rs` (`pub mod <name>;`) and callers import concrete paths.
