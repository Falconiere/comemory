...
//! Mirror test for `src/domains/code/ingest_code.rs`. `crate::domains::code::ingest_code::run` is
...
fn run_inserts_rows_and_reports_files_and_rows() {
    ...
    let resp = crate::domains::code::ingest_code::run(&mut ctx, &body).expect("ingest_code run");
...
fn run_rejects_a_chunk_row_with_a_missing_parent() {
    ...
    let err = crate::domains::code::ingest_code::run(&mut ctx, &format!("{orphan}\n"))
