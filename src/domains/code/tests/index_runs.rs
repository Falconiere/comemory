...
//! Mirror test for `src/domains/code/index_runs.rs`. The history rows are written by
//! REAL `domains::code::index_code` runs over real temp git repos (`common/git_*`) —
...
fn a_data_dir_without_a_database_reports_an_empty_page_and_creates_nothing() {
    ...
    let page = crate::domains::code::index_runs::run(
        &mut ctx,
        crate::domains::code::index_runs::Request::default(),
    )
...
fn the_repo_filter_narrows_the_history_to_one_label() {
    ...
    let mut ctx = Ctx::borrowed(&paths, &cfg, &mut conn);
    let all = crate::domains::code::index_runs::run(
        &mut ctx,
        crate::domains::code::index_runs::Request::default(),
    )
    ...
    let scoped = crate::domains::code::index_runs::run(
        &mut ctx,
        crate::domains::code::index_runs::Request {
...
fn the_window_pages_and_reports_has_more() {
    ...
    let mut ctx = Ctx::borrowed(&paths, &cfg, &mut conn);
    let first = crate::domains::code::index_runs::run(
        &mut ctx,
        crate::domains::code::index_runs::Request {
            repo: None,
            limit: 2,
    ...
    let last = crate::domains::code::index_runs::run(
        &mut ctx,
        crate::domains::code::index_runs::Request {
            repo: None,
            limit: 2,
