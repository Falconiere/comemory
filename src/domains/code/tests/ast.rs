...
//! Mirror test for `src/domains/code/pattern_search.rs`. Calls `crate::domains::code::pattern_search::run` directly against
...
fn run_finds_every_matching_function() {
    ...
    let page = crate::domains::code::pattern_search::run(&mut ctx, req).expect("ast run");
...
fn run_rejects_an_unsupported_language() {
    ...
    let err = crate::domains::code::pattern_search::run(&mut ctx, req)
        .expect_err("unsupported lang must error");
...
fn run_errors_on_a_missing_file() {
    ...
    assert!(crate::domains::code::pattern_search::run(&mut ctx, req).is_err());
}

#[test]
fn request_rejects_unknown_fields() {
    let err = serde_json::from_value::<crate::domains::code::pattern_search::Request>(
        serde_json::json!({
            "pattern": "fn $NAME()",
            "lang": "rs",
            "file": "src/lib.rs",
