...
//! Mirror test for `src/domains/code/hooks.rs`. Real filesystem writes under a real
...
//! `tests/serve__routes__hooks.rs`. This file exercises `crate::domains::code::hooks::run`
//! directly.
...
fn after_install_hooks_all_three_git_rows_report_installed() {
    ...
    let cfg = Config::defaults();
    let mut ctx = Ctx::lazy(&paths, &cfg);
    crate::domains::code::install_hooks::run(
        &mut ctx,
...
fn disable_one_git_hook_leaves_the_other_two_files_byte_identical() {
    ...
    let resp =
        crate::domains::code::hooks::run(&mut ctx, request(&repo, None, Some("post-checkout")))
            .expect("disable run");
...
fn disable_is_idempotent_on_an_already_missing_hook() {
    ...
    let resp =
        crate::domains::code::hooks::run(&mut ctx, request(&repo, None, Some("post-commit")))
...
fn enable_reinstalls_a_previously_disabled_git_hook() {
    ...
    .expect("install-hooks run");
    crate::domains::code::hooks::run(&mut ctx, request(&repo, None, Some("post-merge")))
        .expect("disable run");

    let resp = crate::domains::code::hooks::run(&mut ctx, request(&repo, Some("post-merge"), None))
        .expect("enable run");
...
fn reinforce_toggle_round_trips_through_config_toml() {
    ...
    let resp = crate::domains::code::hooks::run(
        &mut ctx,
        request(
            &repo,
            None,
            Some(crate::domains::code::hooks::REINFORCE_HOOK),
        ),
    )
    ...
    let resp2 = crate::domains::code::hooks::run(
        &mut ctx2,
        request(
            &repo,
            Some(crate::domains::code::hooks::REINFORCE_HOOK),
