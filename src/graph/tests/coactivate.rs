...
fn rerun_does_not_double_count() {
    ...
    materialize(&mut conn, &repo_root, REPO, &BTreeMap::new(), Some(7)).expect("first run");
    let weight_after_first = co_activated_weight(&conn, "aaaaaaa1", "docs/guide.md");
    let access_after_first = access_count(&conn, "aaaaaaa1");

    materialize(&mut conn, &repo_root, REPO, &BTreeMap::new(), Some(7))
        .expect("second run (no-op mine)");
...
fn incremental_commit_accumulates_weight_without_reminting_used() {
    ...
    materialize(&mut conn, &repo_root, REPO, &BTreeMap::new(), Some(7)).expect("incremental run");
