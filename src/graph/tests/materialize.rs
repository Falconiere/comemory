...
fn materialize_writes_edges_ranks_and_cursor() {
    ...
    materialize(&mut conn, &repo_root, "r", &imports_a_to_b(), Some(7)).expect("materialize");
...
fn materialize_accumulates_cochange_incrementally() {
    ...
    materialize(&mut conn, &repo_root, "r", &no_imports, Some(7)).expect("first run");
    // No new commits: the cursor short-circuits mining, weight stays 2.
    materialize(&mut conn, &repo_root, "r", &no_imports, Some(7)).expect("no-op run");
    let weight: i64 = conn
        .query_row(
            "SELECT weight FROM edges WHERE src_id='file:r:a.rs' \
             AND dst_id='file:r:b.rs' AND rel='co_changed'",
    ...
    materialize(&mut conn, &repo_root, "r", &no_imports, Some(7)).expect("incremental run");
    let weight: i64 = conn
        .query_row(
            "SELECT weight FROM edges WHERE src_id='file:r:a.rs' \
             AND dst_id='file:r:b.rs' AND rel='co_changed'",
...
fn materialize_resets_cochange_weights_when_cursor_is_lost() {
    ...
    materialize(&mut conn, &repo_root, "r", &no_imports, Some(7)).expect("lost-cursor run");
    let weight: i64 = conn
        .query_row(
            "SELECT weight FROM edges WHERE src_id='file:r:a.rs' \
             AND dst_id='file:r:b.rs' AND rel='co_changed'",
...
fn materialize_refreshes_imports_as_state_not_accumulation() {
    ...
    materialize(&mut conn, &repo_root, "r", &imports_a_to_b(), Some(7)).expect("first run");
    // Same imports again: still exactly one edge (INSERT OR IGNORE).
    materialize(&mut conn, &repo_root, "r", &imports_a_to_b(), Some(7)).expect("repeat run");
    let count: i64 = conn
        .query_row(
    ...
    materialize(&mut conn, &repo_root, "r", &dropped, Some(7)).expect("dropped-import run");
    let count: i64 = conn
        .query_row(
            "SELECT count(*) FROM edges WHERE src_id='file:r:a.rs' AND rel='imports'",
            [],
...
fn materialize_without_indexed_symbols_is_a_noop_and_keeps_no_cursor() {
    ...
    materialize(&mut conn, &repo_root, "r", &BTreeMap::new(), Some(7)).expect("empty-repo run");
...
fn materialize_on_commitless_repo_errors_and_rolls_back() {
    ...
    let err = materialize(&mut conn, &repo_root, "r", &BTreeMap::new(), Some(7));
    assert!(
        err.is_err(),
