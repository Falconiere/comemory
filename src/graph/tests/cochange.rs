...
fn build_cochange_repo(root: &Path) -> PathBuf {
    let repo = git_sample::build_sample_repo(root);
    git_commit::commit_files(&repo, &[("a.rs", "a v1"), ("b.rs", "b v1")], "c1");
    git_commit::commit_files(&repo, &[("a.rs", "a v2"), ("b.rs", "b v2")], "c2");
    git_commit::commit_files(&repo, &[("b.rs", "b v3"), ("c.rs", "c v1")], "c3");
    ...
    git_commit::commit_files(&repo, &files, "mega formatting sweep");
    repo
}
...
fn incremental_mine_counts_only_commits_after_cursor() {
    ...
    git_commit::commit_files(&repo, &[("a.rs", "a v5"), ("c.rs", "c v5")], "c5");
...
fn commit_at_exact_mega_cap_is_kept_not_skipped() {
    let tmp = TempDir::new().expect("tempdir");
    let repo = git_sample::build_sample_repo(tmp.path());
    ...
    git_commit::commit_files(&repo, &files, "exactly-cap commit");
...
fn first_run_cap_excludes_pairs_buried_below_the_commit_limit() {
    ...
    git_commit::commit_files(&repo, &[("a.rs", "a v1"), ("b.rs", "b v1")], "pairA");
    git_commit::commit_files(&repo, &[("a.rs", "a v2"), ("b.rs", "b v2")], "pairB");
...
fn append_empty_commits(repo: &Path, n: usize) {
    ...
    for i in 0..n {
        ...
        let _ = writeln!(stream, "data {}", msg.len());
        stream.push_str(&msg);
        if i == 0 {
    ...
    git_repo::run_git(repo, &["reset", "--mixed", "-q", "HEAD"]);
}
