...
| `history.rs` | `copy_history_tables` | the run-history half of the preservation copy — `eval_runs` (+ v15's `discarded`), `gc_runs`, `index_runs`, and v16 `sync_log` / `sync_state` / `sync_binding` — called from `copy.rs`'s learning-state pass |
