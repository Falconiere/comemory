...
| --- | --- | --- |
| `detect.rs` | `Detected` | Read-only probe of machine + repo. Delegates to `api::doctor`, `domains::code::hooks`, `domains::code::repos`, `domains::documents::sources`, and `domains::sync::auth_file`; never creates the database and never touches the network |
