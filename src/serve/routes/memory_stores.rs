...
/// `config.toml` (`memory_store::patch`) and answer the updated store.
///
...
/// config through `memory_store::get` (the same shape `routes::config`
...
async fn patch(
    ...
    Path(id): Path<String>,
    Json(req): Json<memory_store::PatchRequest>,
) -> Response {
    let started = Instant::now();
    ...
    let result = run_blocking(move || {
        let _permit = permit;
        {
            let cfg = state.cfg();
            let mut ctx = Ctx::lazy(state.paths(), &cfg);
            memory_store::patch(&mut ctx, &id, &req)?;
        }
...
async fn create(
    State(state): State<AppState>,
    Json(req): Json<memory_store::CreateRequest>,
) -> Response {
    let started = Instant::now();
...
/// (`memory_store::sync`). The body is optional; `{"push": true}` pushes
...
async fn sync(
    State(state): State<AppState>,
    Path(id): Path<String>,
    body: Option<Json<memory_store::SyncRequest>>,
) -> Response {
    let started = Instant::now();
    if let Err(resp) = guard_job("memory-stores.sync", &state) {
        return *resp;
    }
    let req = body.map_or_else(memory_store::SyncRequest::default, |Json(r)| r);
    ...
    let job = jobs::spawn_job_with_id(
        ...
        move |job_id| {
            ...
            let done = memory_store::sync(&mut ctx, &id, &req, |line| {
