...
| `sync/` | cloud-sync client: `auth_file` (`auth.json` **schema v2** — `{version, secret, key_prefix, api_url, organization_id, organization_slug, organization_name, workspace_id, email}`, 0600 atomic I/O; a v1 file is **refused, not migrated**, because the unbound device key it holds no longer authenticates — `load` raises `Error::Usage`, `load_usable` reports it as absent for the best-effort `save`/`context` paths), `client` (platform HTTP over `reqwest::blocking`; sends `Authorization` only — the `X-Comemory-Workspace` header is gone), `skip_repos` (`normalize_repo_label` + a `globset` `SkipMatcher` behind `[sync] skip_repos`, the one client-side filter left after the per-repo GitHub App allowlist was deleted), `push`/`pull`/`verify` (all reading `auth.workspace_id`), `initial` (`run_initial_sync` — the bounded pull-then-push `auth login` runs before returning; pull first so `sync_binding` cannot claim a memory the org already holds), `auto` (`after_save_best_effort` returning a `#[must_use] AutoPush` the CLI **joins** — a detached thread dies with the exiting process, which is why `[sync] after_save` used to be a race), `redact`/`rules.toml` |
...
| `stats/` | feedback **domain logic** — Beta-distribution scoring, counter arithmetic, query-id provenance, the `feedback_events.provenance` vocabulary (`PROV_MANUAL` / `PROV_IMPLICIT` / the `PROV_AUTO_*` tags, plus `feedback::Source`, the feedback routes' `explicit\|implicit` request word and its one mapping onto the stored value; golden harvest and `mine` read `manual` only), and `code_feedback::resolve_identity`'s rule that a missing `code_symbols` row is a loud error while a missing *parent* row falls back to own identity. The SQL for `feedback` / `feedback_events` / `code_feedback` / `index_failures` lives in `store::{feedback,code_feedback,index_failures}`; `sqlite` keeps `StatsDb` as the domain-facing handle (public surface unchanged), opened via the shared `store::connection` |
