...
comemory auth login|status|logout  # cloud workspace-key device login (cmk_ in auth.json)
```
...
| `cloud/` | cloud platform auth for `comemory auth` — `api_url` (`--api-url` > `COMEMORY_API` > `https://api.comemory.io`), `device` (RFC 8628 code/token poll + `POST /v1/device/mint-workspace-key` with Bearer access_token), `credentials` (`auth.json` 0600 atomic I/O; `COMEMORY_API_KEY` overrides the file secret). Slice 1 workspace-key only. CLI-only: listed in `serve::routes::meta::CLI_ONLY` beside `serve` / `upgrade` (no `/api/v1` route) |
...
| `COMEMORY_API_KEY` | Optional override of the `auth.json` workspace-key secret (same idea as CI without writing the file). | unset |
