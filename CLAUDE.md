...
`qwick` is a Rust CLI that fuses engram-style developer memory, grepai-style
...
- **Source of truth:** markdown files with YAML frontmatter at
  `~/.qwick/memories/{id}-{slug}.md` (override with `QWICK_DATA_DIR`).
...
- **AST extraction:** `ast-grep-core 0.38` + `ast-grep-language 0.38` for
  symbol enumeration and user-supplied `qwick ast` patterns.
...
qwick doctor                    # runtime health check
```
...
| `retrieval/` | adaptive `router`, `hybrid` search, `corrective` fallback, `rank` blending, `bundle` for `qwick context` |
| `ast/` | `extractor` (symbol enumeration via tree-sitter through ast-grep), `pattern` (user-facing `qwick ast`), per-language wiring |
...
|----------|---------|---------|
| `QWICK_DATA_DIR` | Root data directory (memories + lancedb + kuzu + sqlite) | `~/.qwick` |
| `QWICK_INDEXING_AUTO_REINDEX` | `lazy` \| `hook` \| `off` — controls automatic code-index refresh | `lazy` |
| `QWICK_RETRIEVAL_TOP_K` | Number of results returned by the hybrid router | `12` |
| `QWICK_RETRIEVAL_MEMORY_THRESHOLD` | Minimum cosine similarity for the memory table | `0.55` |
| `QWICK_RETRIEVAL_CODE_THRESHOLD` | Minimum cosine similarity for the code table | `0.50` |
| `QWICK_GIT_AUTO_SYNC` | `true`/`1` to enable best-effort git commit + push after a save | `false` |
...
3. Best-effort graph upsert (`src/cli/save.rs::upsert_graph`):
   ...
   - `cross_link::extract_refs` walks the body for backtick-fenced
     ...
     File/Symbol nodes do not yet exist — `qwick index-code` fills them in
     later).
...
- The memory body is **not yet** embedded into `lancedb.memory_chunks` from
  the save path itself; rebuild via `qwick index-code` or a future
  dedicated `qwick index` command.
- `RelatesTo` neighbor discovery (spec §8 step 6) is deferred.
- Git auto-sync from `git_utils` runs only when `QWICK_GIT_AUTO_SYNC` is
  enabled.
...
- `tests/common/` carries shared fixtures (temp data-dir builders, gold
  memory samples).
- CLI integration tests use `assert_cmd` against the real `qwick` binary.
...
- `cargo install qwick` (source, from crates.io once published).
- `brew install SidegigLLC/tap/qwick` (Homebrew tap
...
- Prebuilt tarballs for `aarch64-apple-darwin`, `x86_64-apple-darwin`,
  ...
  [GitHub Releases](https://github.com/SidegigLLC/qwick/releases).
