...
| `sync/` | cloud-sync client: `auth_file` (auth.json schema v2; v1 refused), `client` (reqwest+rustls; Authorization only), `skip_repos`, `push`/`pull`/`verify`, `initial` (exhaustive pull-then-push at login), `daemon` + `daemon_unit` + `daemon_templates` (user LaunchAgent `io.comemory.sync` / systemd `--user` `comemory-sync.service`; continuous auto-sync — in-process after_save removed), `redact`/`rules.toml` |
