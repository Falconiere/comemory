...
- **D2 — `fileSize.max` is 300, not the kit's 500.** comemory's module
  ...
  One `fileSize.overrides` entry raises the ceiling to 400 for
  `.github/workflows/release.yml` alone. That file is generated by
  `cargo dist init` and is already 310 code lines; the repo does not choose
  its length and cannot split it, so the ceiling was unenforceable there and
