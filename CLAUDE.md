...
| `sync/` | cloud-sync client: `auth_file` (auth.json schema v2; v1 refused), `client` (reqwest+rustls; Authorization only, plus `ws_ticket`/`channel_url` for the workspace channel), `skip_repos` (the ONE client-side filter left — an empty `repo` label no longer withholds anything, since the label comes from the cwd's git repo and made sync eligibility depend on which directory `save` ran in), `push`/`pull`/`verify`, `push_on_save` (the inline push after a local write, called from `cli::save` / `cli::delete` through `off_runtime` and never from `api::`, which also runs inside `serve`), `initial` (exhaustive pull-then-push at login), `daemon` + `daemon_unit` + `daemon_templates` (user LaunchAgent `io.comemory.sync` / systemd `--user` `comemory-sync.service`, now OPT-IN via `auth login --daemon`; `daemon_wake` is gone with it, since a save pushes itself rather than nudging a sleeping daemon), `redact`/`rules.toml` |
