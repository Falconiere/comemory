...
| `store` | the SQLite chokepoint — the only module importing `rusqlite`, holding every SQL string in the crate (see CLAUDE.md Binding Rule 10). Connection layer, schema_meta, migrations, vector + FTS helpers, identifier tokenizer (camelCase/snake_case split + FFI registration), `edge_fts` (the triplet index over `edges` — rendering, refresh, and the ladder behind `comemory edges`) |
...
| `stats` | feedback domain logic — Beta scoring and counter arithmetic; the SQL lives in `store::{feedback,code_feedback,index_failures}` |
