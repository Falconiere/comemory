...
| `utilities` | transport-neutral shared primitives every domain and both delivery adapters may use: the execution `context` (`Ctx`), `pagination` (`Page`, `PageWindow`, `PageMeta`), `id_list`, `when`, `ref_args`, `embedding_input` + `vector_stdin`, `embed`, `fetch`, `http_error`, `simhash`, `digest`, `file_lock`, `path_containment`, `progress` (`ProgressSink`), `query_id`, and `telemetry` |
