...
| `domains/architecture/` | the architecture capability: a component-level model of one repository, scaffolded deterministically from `graph::query::build_code_graph` plus `store::indexed_files`, enriched by an agent, validated against the indexed paths, and stored as a single memory tagged `architecture` (so it syncs and `console.comemory.io` can draw it from `GET /api/v1/memories?tag=architecture`). `check` diffs the stored model against a fresh scaffold and reports stale members, unmapped clusters and missing mined edges without failing; `mermaid` renders it for a terminal or a document; `learn` hands the scaffold to an agent command the caller names — there is still no in-process LLM |
