...
| `sync.daemon_interval` | Sleep between cycles of the opt-in daemon (`comemory auth login --daemon`). A cycle whose pass ends with `more` runs the next pass at once instead of sleeping. | `"5s"` |
