...
| | | Two `entity_kind`s are carried today: `memory` and `code_generation`. A code generation's `entity_key` is the repo label and its payload is the whole projection (manifest, snippet-free symbols, edges) — accepted as one unit or not at all. One planned against a parent the repo has moved past is `409`, never merged; a `tombstone` for it means the repository is authoritatively empty, never that the sender lost its checkout |
