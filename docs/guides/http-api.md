...
| ○ `GET /graph/nodes/{id}/source` | Reads up to 1 MiB of UTF-8 source from the indexed local worktree, contained to that repo root. Returns `{content, reason}`; `content: null` explains an absent worktree or file. Synced cloud projections have no local worktree. |
