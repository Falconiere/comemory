...
| Recursive/nonrecursive CTEs, set operations, subquery expressions, and table-valued sources | `edges_retrieval` graph walks; `edges` neighbor queries; `code_graph_nodes` lookup unions; set-based cleanup in `prune_apply`, `repo_drop`, and `code_row`; `replica_redaction`'s `UNION` of feed, revision and event-table subqueries that picks the payloads retention expires; `json_each` and `pragma_table_info` | 0.8.0 ([#110](https://github.com/Falconiere/toolu-orm/issues/110)) |
