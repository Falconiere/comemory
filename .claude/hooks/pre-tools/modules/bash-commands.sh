...
if echo "$cmd_only" | grep -qE '(^|[[:space:]]|&&|\|\||;|`|\()(npm|npx|yarn|pnpm|bun|bunx|pip|uv|poetry)[[:space:]]'; then
  deny_pre "comemory is a Rust project — use cargo / just / scripts/* instead of npm|bun|pip|uv."
  exit 0
fi
...
if echo "$cmd_only" | grep -qE '(^|[[:space:]]|&&|\|\||;)(rustfmt([[:space:]]|$)|cargo[[:space:]]+fmt([[:space:]]|$)|cargo[[:space:]]+clippy([[:space:]]|$))'; then
