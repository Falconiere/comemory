...
| `0027_replica_exchange.sql` | v27: the `replica-v1` exchange client (#255) — `sync_exchange` (per `(api_url, workspace)` key: negotiated protocol, network state, upgrade/replay/stall markers), `sync_policy_snapshot` (the policy last loaded for a key), `replica_pull_hold` (pulled positions passed without applying, and why), `replica_binding` (what the upstream last held for each exchanged entity), `replica_replay` (compacting-replay scratch rows); `replica_cursor` rebuilt keyed by `(api_url, workspace_id)` with an anchor; `replica_operation` gains hold, stamp and wire columns |
| `0028_historical_document_revision.sql` | v28: marker for the Rust post-pass that repairs databases written by the short-lived `0026_repository_approval` branch. That branch’s document table used `state` / `created_at` / `activated_at`; the post-pass retains active revisions and their passages, removes staged and superseded rows, and rebuilds the table in the released `accepted_at` shape. The migration is destructive so preflight requires a successful snapshot |
