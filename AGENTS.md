...
| `api/` | the staged shell holding what is left of the shared command core between `cli::` and `serve::routes::`: `api::<cmd>::run(&mut Ctx, Request)` holds each subcommand's logic — a *move* of each `cli::<cmd>::run`'s middle (arg-parsing and TTY/`--json` rendering stay in `cli::`), so neither surface duplicates it (reuse precedent: `retrieval::code_search::search_code_hits`, generalized here to every command). Every `Request` derives `#[serde(deny_unknown_fields)]`, enforced by `tests/api__parity.rs`'s clap-introspection walk. `utilities::context::Ctx` bundles `Paths` + `Config` with a connection that is either `Borrowed` (the CLI's own connection, or the server's shared per-request one) or `Lazy` (opened on first `Ctx::conn()` call — a job worker's own dedicated connection; conn-free commands like `install`, `doctor`, `rebuild`, `ast`, `install-hooks`, `completions` never open one at all). cwd-dependent middles (`save --ref-*` anchoring, the code rerank's working-set prior) resolve against the *calling process's* cwd — the server's cwd over HTTP, not the HTTP client's, documented behavior rather than a bug. Only the two integrations cores are still here, both CLI-only: `install` (+ `install/bundle`, the embedded agent bundle) and `setup` (+ `setup/`, the detect → plan → apply phases). #175 takes both into `domains/integrations/` and #178 deletes the shell. Already gone: the code cores — `ast` (now `pattern_search`), `index_code`, `ingest_code`, `index_runs`, `repos`, `repo_admin`, `hooks`, `install_hooks` — to `domains/code/` with #167; the document cores `index`, `sources`, `unindex` to `domains/documents/` with #168; the memory cores — `save`, `delete`, `list`, `show`, `update`, `restore`, `trash`, `refresh_refs` — to `domains/memories/` with #169; the graph cores — `graph` (now `view`), `graph_nodes`, `graph_recompute`, `edges` — to `domains/graph/` with #170; the retrieval cores — `search`, `search_code`, `context`, `find`, `suggest`, `config_retrieval` — to `domains/retrieval/` with #171; the sync cores `sync` (now `exchange`) and `memory_store` to `domains/sync/` with #172; the learning cores — `feedback`, `eval`, `mine`, `tune`, `bandit`, plus the console-only `learning` (now `console`) and `learning_proposals` — to `domains/learning/` with #173; and the maintenance cores — `doctor` (+ `doctor/`), `gc`, `prune`, `consolidate`, `rebuild` (+ `rebuild/`), `stats`, plus the console-only `gc_policy`, `overview` and `reembed` — to `domains/maintenance/` with #176 |
...
| `output/` | TTY (`owo-colors`) and JSON (`serde_json`) emitters, shared between subcommands. CLI presentation only: each retrieval command's `--json` envelope and rows are what both transports serialize, so they live in `domains::retrieval::{search_result, code_search_result, context_result, scope}` and the explain strip in `domains::retrieval::explain`; `output::{search, search_code, context}` keep `emit`/`write_tty` (#171) |
...
- **D4 — `src.nested` replaces the starter map** with the exact, project-specific
  ...
  - `domains/*`: `ast`, `cloud`, `consolidation`, `doctor`, `document`, `evaluation`, `exchange`, `index_code`, `memory_store`, `rebuild`, `repos`, `retention`, `source`, `unified`, `upgrade`, `tests`, `proptest-regressions`
