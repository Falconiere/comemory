...
2a. **Prior lookup** (`MemoryStore::prior`, `src/memory/prior.rs`), run
...
5a. **Inline cloud push** (`cli::save` only, not `api::save`): when
   ...
   `api::save` also runs inside `comemory serve`, where `reqwest::blocking`
...
- **D4 — `src.nested` replaces the starter map** with the exact, project-specific
  ...
  - `domains/*`: `ast`, `index_code`, `repos`, `tests`, `proptest-regressions`
...
- **D5 — `src.requireReadme` replaces the pinned single-`domains` rule during staging with exactly 40 folders.**
