...
| `stats/` | feedback **domain logic** — Beta-distribution scoring, counter arithmetic, query-id provenance, the `feedback_events.provenance` vocabulary (`PROV_MANUAL`, `PROV_IMPLICIT`, the `PROV_AUTO_*` tags, and `feedback::Source` — the routes' `explicit\|implicit` word mapped onto the stored value), and `code_feedback::resolve_identity`'s rule that a missing `code_symbols` row is a loud error while a missing *parent* row falls back to own identity. The SQL for `feedback` / `feedback_events` / `code_feedback` / `index_failures` lives in `store::{feedback,code_feedback,index_failures}`; `sqlite` keeps `StatsDb` as the domain-facing handle (public surface unchanged), opened via the shared `store::connection` |
