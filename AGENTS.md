...
comemory install-hooks --repo P    # hook a repo once: every HEAD move there indexes + syncs it (no cd afterwards)
...
comemory sync                      # drain the bound key both ways until caught up (replica-v1, or the old protocol + code index)
comemory sync --action auto        # the pass hooks fire: refresh every stale hooked repo, then drain (no login needed)
...
| `cli/sync_auto.rs` | `comemory sync --action auto` — the CLI half of `domains::sync::auto`, dispatched by `cli::sync` before any login is required. With the required coordinator (#257) it sends one wake over the control socket and returns; only under the `COMEMORY_SYNC_DAEMON=0` harness switch does it fall back to running the pass off the runtime itself (`domains::code::hooked_refresh` is the local half of that fallback — index the `--path` checkout, re-index every stale hooked repo; passes serialize on `sync.lock` and coalesce through the one-slot `sync-auto.queue`). Prints nothing unless `--json` either way (its callers are git hooks and agent hooks) |
| `cli/sync_daemon.rs` | `comemory sync daemon {ensure,status,restart,repair,stop,uninstall,run}` — the required resident coordinator's lifecycle (#257); `install`/`start` are hidden deprecated aliases of `repair`/`ensure`. `run` is the supervisor's foreground entry point (`domains::sync::daemon::run_foreground`); every other verb calls `domains::sync::daemon::ensure`/`status_view` off the runtime |
| `cli/daemon_preflight.rs` | Runs first in `cli::run`, before every ordinary command: an exhaustive match over `Cmd` classifies each as `Exempt` (lifecycle/introspection — never touches the coordinator), `BestEffort` (`auth logout` — verifies/repairs but never fails the command), or `Required` (everything else, including `watch`/`mcp` — fails with a named local-service error, exit 69, if no coordinator can be reached). A no-op under `COMEMORY_SYNC_DAEMON=0` |
| `cli/watch.rs` | `comemory watch` — with the required coordinator running, attaches to it instead of holding its own connection: `--once` subscribes then asks for one `catch_up`; following instead reports one `pulled` per `pass_finished` event, re-attaching (jittered backoff, 1s → 30s) on a dropped subscription, a coordinator restart included. Only under `COMEMORY_SYNC_DAEMON=0` does it hold the platform's workspace channel itself, as before #257: mints a 60-second ticket, holds it open (`tokio-tungstenite`, async on the existing runtime), and drains the pull direction (`domains::sync::drain`, pass after pass until no `more`) on every `hello`/`change` frame — nothing is read out of a frame except that it arrived, so a missed one costs latency and a duplicate costs an empty pull |
...
| `domains/sync/` | the sync capability (#172): the org-scoped credential and the login that mints it — `auth_file` (auth.json schema v2; v1 refused; `load_usable` is the read-only probe `domains::integrations::setup::detect` uses), `cloud` + `cloud/` (`api_url` — `--api-url` > `COMEMORY_API` > `https://api.comemory.io` — and `device`, the RFC 8628 code/token poll plus `POST /v1/device/mint-org-key`; the device-code grant is only the approval channel, so the workspace travels *in the key* and a mint missing `organizationId`/`workspaceId` is refused rather than written), and `login` (the three `comemory auth` sequences with the progress writer injected and nothing rendered); the wire protocol's two halves — `client` (reqwest+rustls; Authorization only, plus `ws_ticket`/`channel_url` for the workspace channel), `client_code`, and `exchange` + `exchange/` (the *server* half the `serve` routes call: the wire models plus the `changes` / `manifest` / `import` / code-import cores); the exchange client — `drain` + `drain/` (#255: the one entry point every caller drains through — session open and protocol negotiation keyed by `(api_url, workspace)`, the `replica-v1` pass and the legacy pass under one loop, budget and network state, holds, compacting replay, rebootstrap, verify and the `exchange` status block); the directions of travel — `push`/`pull` (the legacy pass's batch and page), `verify`, `initial` (the drain at login, then the legacy code push), `manual` (what one `comemory sync` run does), `push_on_save` (one push-only pass after a local write, skipped when a pass holds `sync.lock`, called from the CLI seam only so an HTTP write starts no outward sync), and `code` + `code_plan` + `client_code` (the code-index push: every indexed repo's snippet-free projection — paths, blob OIDs, symbol names and line ranges, `imports`/`co_changed` edges — diffed by blob OID against the workspace's `GET /v1/sync/code/manifest` and posted to `POST /v1/sync/code/import`); the filters and the resident loop — `redact`/`rules.toml` (the curated secret scan before a push is enqueued), `skip_repos` (the ONE client-side filter left — an empty `repo` label no longer withholds anything, since the label comes from the cwd's git repo and made sync eligibility depend on which directory `save` ran in), `watch` (the raw platform-channel follower `channel_loop`/`Frame` — the harness-switch fallback `cli::watch` uses directly, and what the coordinator's own `daemon::channel` wraps), `auth_barrier` (the durable logout barrier, `auth.disabled`: every credential read answers logged out while it stands, cleared only by an explicit login), and `daemon` + `daemon/` + `daemon_unit` + `daemon_templates` (#257: the **required**, always-resident per-directory coordinator every ordinary command's preflight verifies and repairs — see [`domains/sync/daemon/README.md`](src/domains/sync/daemon/README.md) for its control-socket protocol, lifecycle and supervisor pieces; `daemon_unit` is now just the `launchctl`/`systemctl` shell-out `daemon::supervisor` shares, and `daemon_templates` the per-directory plist/unit text); and `memory_store` + `memory_store/` — a different kind of sync: Git synchronization of the data directory's own work tree, with its own `store-sync` job and log contract. `replica/` is the `replica-v1` journal protocol and its engine halves for every entity kind — memories, code generations, document revisions and, since #254, feedback and activity events (`activity_payload`, `event_capture`, `event_accept`, `validate_events`), each counted once per event whatever operation id carries it. Clap flags, prompts and every rendered line stay in `cli`; all SQL stays in `store::{sync_log,sync_state,sync_binding,code_sync,sync_exchange,sync_policy_snapshot,replica_*}` |
...
5a. **Inline cloud push** (`cli::save` only, not `domains::memories::save`): when
   `[sync] push_on_save` is on and `auth.json` is usable, run one push-only
   drain pass (`domains::sync::drain`) under `push_on_save_timeout`, skipping
   it when a pass already holds `sync.lock`. When that push does not fully
   drain the outbox, sends a best-effort wake to the required coordinator
   (#257) so the entry does not wait for its next tick. Never fails the
   save — offline, unauthenticated and refused all leave the entry in the
   outbox for that wake, or the next `sync` / `watch` / coordinator cycle
   either way. It lives in `cli::` because
...
- **D4 — `src.nested` replaces the starter map** with the exact, project-specific
  ...
  - `domains/*`: `ast`, `cloud`, `consolidation`, `daemon`, `doctor`, `document`, `drain`, `evaluation`, `exchange`, `index_code`, `install`, `memory_store`, `replica`, `rebuild`, `repos`, `retention`, `setup`, `source`, `unified`, `upgrade`, `tests`, `proptest-regressions`
  ...
  project ownership.
- **D5 — `src.requireReadme` replaces the pinned single-`domains` rule during staging with exactly 46 folders.**
