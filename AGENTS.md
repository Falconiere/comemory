...
| `api/` | the emptied command-core shell. `api::<cmd>::run(&mut Ctx, Request)` once held each subcommand's logic so `cli::` and `serve::routes::` did not duplicate it; every core has now moved under `domains::`, #175 taking the last two — `install` (+ `install/bundle`) and `setup` (+ `setup/`) — into `domains/integrations/`. Nothing is declared here any more and #178 deletes the module. The two facts that outlived it: every `Request` derives `#[serde(deny_unknown_fields)]`, enforced by `tests/api__parity.rs`'s clap-introspection walk; and `utilities::context::Ctx` bundles `Paths` + `Config` with a connection that is either `Borrowed` (the CLI's own, or the server's shared per-request one) or `Lazy` (opened on first `Ctx::conn()` call — a job worker's own dedicated connection; conn-free commands like `install`, `doctor`, `rebuild`, `ast`, `install-hooks`, `completions` never open one at all). cwd-dependent middles (`save --ref-*` anchoring, the code rerank's working-set prior) resolve against the *calling process's* cwd — the server's cwd over HTTP, not the HTTP client's, documented behavior rather than a bug |
