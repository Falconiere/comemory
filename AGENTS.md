...
| `git_utils.rs` | blob OID / HEAD / branch / remote lookups, git-hook installation helpers, and `repo_label` — the ONE rule turning a checkout into a repo label: the **main** worktree's basename (`commondir()`'s parent), so every linked `git worktree` files under its parent repo's name instead of minting a `<worktree-dir>` repo. Every label the binary infers goes through it (`cli::lazy_reindex`, the `REINDEX_HOOK_SCRIPT` git hook, `code_rerank::WorkingSet::from_cwd`, `api::index`'s document sources, `POST /repos`' default), and it mirrors the plugin wrapper's `integrations/agent/lib/repo-scope.sh`. `hooks_dir` is the matching rule for hooks: `<commondir>/hooks`, where git actually runs them from a linked worktree |
| `utilities/` | transport-neutral shared primitives, usable by every domain and by both delivery adapters, one concern per file: `context` (the `Ctx` every command core takes), `pagination` (`Page<T>`, `PageWindow`, `PageMeta`, `page_window`, `page_meta`), `id_list` (CSV split/dedupe + the two id-list parsers), `when` (`--since`/`--until`/`--as-of` values), `ref_args` (`--ref-file`/`--ref-symbol` → `References`), `embedding_input` (pure `--vector` CSV / JSON payload decoding) and `vector_stdin` (the process-stdin acquisition split out of it), `embed`, `fetch`, `http_error`, `simhash`, `digest` (SHA-256 hex + the lowercase-hex shape check), `file_lock`, `path_containment`, `progress` (`ProgressSink`), `query_id`, and `telemetry` (the persisted `retrieval_log.source` / `feedback_events.{target_kind,provenance}` vocabularies). Nothing here may import `cli`, `serve`, or `output` |
...
- **D5 — `src.requireReadme` replaces the pinned single-`domains` rule during staging with exactly 38 folders.**
