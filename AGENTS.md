...
| `cli/` | clap subcommand entry points + the top-level dispatcher in `cli.rs`, plus `completion_script` (completion-script generation for both `comemory completions` and `GET /api/v1/completions`) and `pagination` (the flattened `--limit`/`--offset` clap struct). The shared date-flag layer moved to `utilities::when` with #166; `edges` is the fourth free-text surface (`comemory edges <query>` — lexical search over `edge_fts`, self-healing an empty index on first use) |
...
| `api/setup/` | the three phases behind `comemory setup`, split so every decision is testable without a terminal: `detect` (read-only probe — delegates to `api::doctor`, `domains::code::hooks`, `domains::code::repos`, `domains::documents::sources`, and `sync::auth_file`; guarded on `comemory.db` already existing so a probe never CREATES the store, and reads the credential from `auth.json` rather than asking the platform, so detection never touches the network), `plan` (pure `(Detected, Request) -> Vec<Step>` — where every decision the wizard appears to make actually lives), and `apply` (the only phase that writes; a per-step error becomes that step's `Failed` state rather than the run's, so one failure cannot hide the others). `STEP_IDS` in `api/setup.rs` is a public contract: `--only`/`--skip` name them and the JSON envelope carries them |
...
| `api/` | shared command core between `cli::` and `serve::routes::`: `api::<cmd>::run(&mut Ctx, Request)` holds each subcommand's logic — a *move* of each `cli::<cmd>::run`'s middle (arg-parsing and TTY/`--json` rendering stay in `cli::`), so neither surface duplicates it (reuse precedent: `retrieval::code_search::search_code_hits`, generalized here to every command). Every `Request` derives `#[serde(deny_unknown_fields)]`, enforced by `tests/api__parity.rs`'s clap-introspection walk. `utilities::context::Ctx` bundles `Paths` + `Config` with a connection that is either `Borrowed` (the CLI's own connection, or the server's shared per-request one) or `Lazy` (opened on first `Ctx::conn()` call — a job worker's own dedicated connection; conn-free commands like `doctor`, `rebuild`, `ast`, `install-hooks`, `install`, `completions` never open one at all). One file per subcommand (`api::search`, `api::find`, `api::prune`, …) plus the console-compat surfaces `api::stats` / `api::find`, plus three directory modules whose donor files sat near the 300-line ceiling: `graph/`, `doctor/` (+ `checks`, one function per health probe), `rebuild/` (+ `copy` and `documents`, the code-index/learning-state/document-domain ATTACH-copy run before the atomic DB swap, which itself snapshots the still-live DB to `comemory.db.pre-rebuild.bak` first). cwd-dependent middles (`save --ref-*` anchoring, the code rerank's working-set prior) resolve against the *calling process's* cwd — the server's cwd over HTTP, not the HTTP client's, documented behavior rather than a bug. The console-only cores (no CLI subcommand of their own, reached only through `serve::routes`) sit beside them: `overview`, `suggest`, `graph_nodes`, `graph_recompute`, `learning`, `learning_proposals`, `config_retrieval`, `reembed`, `gc_policy`, `memory_store`. The code-capability cores — `ast` (now `pattern_search`), `index_code`, `ingest_code`, `index_runs`, `repos`, `repo_admin`, `hooks`, `install_hooks` — moved to `domains/code/` with #167, and the memory cores — `save`, `delete`, `list`, `show`, `update`, `restore`, `trash`, `refresh_refs` — to `domains/memories/` with #169 |
| `serve/` | loopback `/api/v1` HTTP server (`comemory serve`, API-only — the embedded web viewer was removed): axum `router` (mounts `routes::v1_router` behind the path-aware `guard` middleware — enveloped `401`/`403` JSON on `/api/v1/*`, plain text on any other `/api/*` path — and the 5 MiB `BODY_LIMIT`), `scope` (`RepoScope` — the per-request default `repo` filter: `X-Comemory-Repo` header first, the server's `--repo` second, never overriding an explicit parameter), `repo_root` resolution (also used by `retrieval::code_ref_fetch`), `security` (session token generation/matching and the loopback Host guard; the canonicalize-and-contain checks `resolve_within` and `contain_abs` are transport-neutral and live in `utilities::path_containment`), `envelope` (the `{ok,data,meta}` / `{ok,error,meta}` `/api/v1` response envelope plus the one `Error → (StatusCode, code)` mapping table every HTTP error and every failed job's `{code, message}` derives from, with an optional structured `error.details`). `routes/` and `jobs/` are documented in their own rows below |
...
| `cloud/` | cloud platform auth for `comemory auth` — `api_url` (`--api-url` > `COMEMORY_API` > `https://api.comemory.io`), `device` (RFC 8628 code/token poll + `POST /v1/device/mint-org-key` with Bearer access_token, plus `org_status`). The device-code grant is only the approval channel: what it mints is scoped to the organization the user approved in the console, so the workspace travels *in the key* and no request names one. A mint missing `organizationId`/`workspaceId` is refused rather than written. CLI-only: listed in `serve::routes::meta::CLI_ONLY` beside `serve` / `upgrade` (no `/api/v1` route) |
| `capture/` | client-side session capture and distillation (platform Slices 3–4): Claude Code JSONL → explicit-save extractor (`claude-code-explicit-save`), client redaction attestation, `POST /v1/sessions/{id}/candidates`. CLI: `comemory distill` (CLI-only beside `auth` / `sync`). Capture receipts / SessionEnd are #121 |
| `sync/` | cloud-sync client: `auth_file` (auth.json schema v2; v1 refused), `client` (reqwest+rustls; Authorization only, plus `ws_ticket`/`channel_url` for the workspace channel), `skip_repos` (the ONE client-side filter left — an empty `repo` label no longer withholds anything, since the label comes from the cwd's git repo and made sync eligibility depend on which directory `save` ran in), `push`/`pull`/`verify`, `push_on_save` (the inline push after a local write, called from `cli::save` / `cli::delete` through `off_runtime` and never from `api::`, which also runs inside `serve`), `code` + `code_plan` + `client_code` (the code-index push: every indexed repo's snippet-free projection — paths, blob OIDs, symbol names and line ranges, `imports`/`co_changed` edges — diffed by blob OID against the workspace's `GET /v1/sync/code/manifest` and posted to `POST /v1/sync/code/import`; runs from `initial`, `comemory sync`, the daemon when the `code_sync:<repo>` cursor moved, and the tail of a CLI `index-code`), `initial` (exhaustive pull-then-push at login, then the code push), `daemon` + `daemon_unit` + `daemon_templates` (user LaunchAgent `io.comemory.sync` / systemd `--user` `comemory-sync.service`, now OPT-IN via `auth login --daemon`; `daemon_wake` is gone with it, since a save pushes itself rather than nudging a sleeping daemon), `redact`/`rules.toml` |
| `graph/` | graph **algorithms** over the `edges` relation — the table's CRUD and every recursive-CTE walk live in `store::edges` / `store::edges_retrieval`, which these call. `cross_link` reference extraction, `cochange` (git-history co-change mining), `imports` (per-language import edges), `pagerank` (deterministic weighted PageRank), `materialize` (writes `rank_score` onto `code_symbols`), `memory_rank` (the same PageRank over the memory graph — direct memory→memory relations plus in-memory co-citation edges, hub rels excluded — written onto `memories.rank_score`), `coactivate` (commit co-activation reward: a commit touching a memory's referenced files reinforces it), `doc_link` (deterministic `member_of_source`/`references_document` link deriver), `search_edit` (search→edit lookback feeding `auto_search_edit` provenance), `derived` (`refresh_derived_best_effort` — the single post-write pass that refreshes *both* derived artifacts, `memories.rank_score` and the `edge_fts` triplet index, independently best-effort; called at the four seams `save`, `delete`, `rebuild`, and `index-code`), `neighbors` (the one-hop undirected `imports`/`co_changed` file neighborhood query shared by `retrieval::bundle` and `GET /api/v1/graph/nodes/{id}/neighbors`) |
...
| `stats/` | feedback **domain logic** — Beta-distribution scoring, counter arithmetic, `feedback::Source` (the routes' `explicit\|implicit` word mapped onto the stored value), and `code_feedback::resolve_identity`'s rule that a missing `code_symbols` row is a loud error while a missing *parent* row falls back to own identity. The SQL for `feedback` / `feedback_events` / `code_feedback` / `index_failures` lives in `store::{feedback,code_feedback,index_failures}`; `sqlite` keeps `StatsDb` as the domain-facing handle (public surface unchanged), opened via the shared `store::connection` |
...
| `output/` | TTY (`owo-colors`) and JSON (`serde_json`) emitters, shared between subcommands, plus `explain` (the console's explain strip: a hit's `score_parts` rendered as `{name, value, share, note}` rows, `share` being the log-magnitude partition of the multiplicative priors) |
...
| `utilities/` | transport-neutral shared primitives, usable by every domain and by both delivery adapters, one concern per file: `context` (the `Ctx` every command core takes), `pagination` (`Page<T>`, `PageWindow`, `PageMeta`, `page_window`, `page_meta`), `id_list` (CSV split/dedupe + the two id-list parsers), `when` (`--since`/`--until`/`--as-of` values), `ref_args` (`--ref-file`/`--ref-symbol` → `References`), `embedding_input` (pure `--vector` CSV / JSON payload decoding) and `vector_stdin` (the process-stdin acquisition split out of it), `embed`, `fetch`, `http_error`, `simhash`, `digest` (SHA-256 hex + the lowercase-hex shape check), `file_lock`, `path_containment`, `progress` (`ProgressSink`), `query_id`, `repo_root` (the one repository resolver — `file:<repo>:<path>` id to a contained absolute path, its `parse_id` decoder, and the `--root` override map — shared by `cli`, `serve`, retrieval freshness and reference refresh since #167), and `telemetry` (the persisted `retrieval_log.source` / `feedback_events.{target_kind,provenance}` vocabularies). Nothing here may import `cli`, `serve`, or `output` |
| `domains/` | business capabilities, one folder each, declared from `src/domains.rs`. A capability owns its models, algorithms and command cores; it may depend on `store`, `config`, `errors`, `prelude`, `utilities`, and — only through the directed table in `scripts/architecture-policy.json` — another capability. It may never import `cli`, `serve`, `output`, or the legacy `api` tree, and may never exist as an empty scaffold (the architecture gate fails one) |
...
2a. **Prior lookup** (`MemoryStore::prior`, `src/domains/memories/prior.rs`), run
   ...
   `created: false`. The same lookup backs `api::sync::import_state`'s
...
- **D5 — `src.requireReadme` replaces the pinned single-`domains` rule during staging with exactly 41 folders.**
