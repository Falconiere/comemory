...
- **No in-process LLM.** All ranking is deterministic (RRF fusion of FTS5 +
  ...
  this process: see `docs/guides/learned-reranking.md`. The adapter that stage
  can load is trained offline and by hand by `integrations/training/` (#214),
  which is external, opt-in, never triggered by an event, and whose
  qualification records `go` / `no-go` / `insufficient-evidence` against budgets
  declared before any arm is scored.
...
| `embed.rs` | shared embed-command shell-out (single-file module, no children) — runs `COMEMORY_EMBED_CMD` / `--embed-cmd` as `sh -c <cmd>`, feeds the query on stdin, parses `{"embedding":[..]}`. Consumed by `serve` (`POST /api/v1/doctor/reembed`, the server-side re-vectorizing job; `GET /health` reports `embed_cmd_configured`). Since #211 it runs on `utilities::process_runner`, so `EMBED_TIMEOUT` is an end-to-end budget over startup, concurrent stdin/stdout handling and exit rather than the stdout read alone, and a child that never drains a large stdin can no longer deadlock the caller |
...
| `utilities/` | transport-neutral shared primitives, usable by every domain and by both delivery adapters, one concern per file: `context` (the `Ctx` every command core takes: `Paths` + `Config` with a connection that is either `Borrowed` — the CLI's own, or the server's shared per-request one — or `Lazy`, opened on the first `Ctx::conn()` call, which is a job worker's own dedicated connection; conn-free commands like `install`, `doctor`, `rebuild`, `ast`, `install-hooks` and `completions` never open one at all. cwd-dependent middles — `save --ref-*` anchoring, the code rerank's working-set prior — resolve against the *calling process's* cwd, which over HTTP is the server's and not the client's: documented behavior rather than a bug), `pagination` (`Page<T>`, `PageWindow`, `PageMeta`, `page_window`, `page_meta`), `id_list` (CSV split/dedupe + the two id-list parsers), `when` (`--since`/`--until`/`--as-of` values), `ref_args` (`--ref-file`/`--ref-symbol` → `References`), `embedding_input` (pure `--vector` CSV / JSON payload decoding) and `vector_stdin` (the process-stdin acquisition split out of it), `embed`, `fetch`, `http_error`, `simhash`, `digest` (SHA-256 hex + the lowercase-hex shape check), `file_lock`, `path_containment`, `progress` (`ProgressSink`), `query_id`, `repo_root` (the one repository resolver — `file:<repo>:<path>` id to a contained absolute path, its `parse_id` decoder, and the `--root` override map — shared by `cli`, `serve`, retrieval freshness and reference refresh since #167), `telemetry` (the persisted `retrieval_log.source` / `feedback_events.{target_kind,provenance}` vocabularies), `dated_id` (the `<prefix>-<yyyymmdd>-<8hex>` shape both `query_id`'s `q-` ids and the reranker's `rr-` request ids are minted and validated through), and the #211 inference boundary — `process_runner` + `process_pipes` (one child under a single end-to-end deadline spanning startup, concurrent stdin/stdout/stderr handling and exit, with byte-bounded input, stdout and diagnostic stderr, always killing and reaping the process it owns; program and arguments are passed separately so payload text can only reach a child on stdin) and the four reranker files `rerank_protocol` / `rerank_outcome` / `rerank_validate` / `rerank_runner` (the versioned JSON stdin/stdout scorer contract, its typed result vocabulary, full response validation with rank tie-breaking, and the bounded runner that composes them). Nothing on the search path calls the reranker yet — #213 wires it, #212 supplied the optional reference backend in `integrations/reranker/`, and #214 the offline LoRA recipe and qualification harness in `integrations/training/` that produces an adapter for it: a pinned Transformers cross-encoder with PEFT adapter loading, plus a deterministic `lexical-overlap` mode whose model label (`lexical-overlap@1`) no request expecting a real model can carry, so `src/utilities/tests/rerank_runner_2.rs` proves protocol conformance through a real `python3` child on every run without downloading anything. The wire contract is `docs/designs/2026-09-18-reranker-command-protocol.md` § Wire protocol; the backend is `docs/designs/2026-09-18-reranker-reference-backend.md`. Nothing here may import `cli` (the `cli::output` writers included) or `serve`, and a reference into a capability is a `shared layer dependency` failure unless `scripts/architecture-policy.json` declares it: `id_list` validates a memory id through `domains::memories::id`, and `ref_args` resolves blob OIDs through `domains::code::git_utils` and produces `domains::memories::{Ref, References}` |
...
`python3` is a prerequisite of four suites. `src/utilities/tests/rerank_runner_2.rs` drives the shipped reranker backend in `integrations/reranker/` as a real child process over real pipes, in its deterministic `lexical-overlap` mode, so it needs no model and no Python package — but it does need the interpreter, which CI's `ubuntu-22.04` runner ships. It deliberately FAILS rather than skipping when `python3` is absent: a skipped protocol test that reads as a pass is exactly the failure mode these gates exist to prevent. The model-dependent half is a separate opt-in suite, `bash integrations/reranker/run-model-tests.sh`, which refuses to exit 0 unless every pinned library version and the pinned snapshot are present.

`tests/lora_qualification.rs` and `tests/lora_qualification_2.rs` do the same for the offline LoRA recipe in `integrations/training/` (#214), sharing `tests/common/lora_harness.rs`: a real export through `comemory export-dataset`, a generated benchmark set, a real `comemory benchmark`, and real `python3` children scoring arms over the wire protocol in the same deterministic `lexical-overlap` mode — which the decision refuses a `go` by model label, because it measures no relevance quality. The second of them also runs `integrations/training/tests/test_offline_recipe.py` through `python3 -m unittest`, whose centrepiece hands a synthetic adapter package to the reference backend's own `validate_adapter`. Both fail rather than skip without `python3`. The opt-in counterpart is `bash integrations/training/run-training-tests.sh`, which reuses the reranker's preflight — the two `requirements.txt` files pin the same seven versions, deliberately and provably — and exits 69 with "the training suite did NOT run" rather than 0.
