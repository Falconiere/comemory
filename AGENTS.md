...
| `cli/sync_auto.rs` | `comemory sync --action auto` — the CLI half of `domains::sync::auto`: dispatched by `cli::sync` before any login is required, runs the pass off the runtime, and prints nothing unless `--json` (its callers are git hooks and agent hooks). `domains::code::hooked_refresh` is the local half (index the `--path` checkout, re-index every stale hooked repo); passes serialize on `sync.lock` and coalesce through the one-slot `sync-auto.queue` |
| `cli/watch.rs` | `comemory watch` — the only long-lived CLI command: mints a 60-second ticket, holds the platform's workspace channel open (`tokio-tungstenite`, async on the existing runtime), and drains the pull direction (`domains::sync::drain`, pass after pass until no `more`) on every `hello`/`change` frame. Frames are nudges: nothing is read out of one except that it arrived, so a missed frame costs latency and a duplicate costs an empty pull. Reconnects with jittered backoff (1s → 30s) |
...
5a. **Inline cloud push** (`cli::save` only, not `domains::memories::save`): when
   ...
   it when a pass already holds `sync.lock`. Never fails the save —
...
- The test profile uses `opt-level = 1` for the real indexing/replication
  workloads, retaining debug assertions and overflow checks. Nextest starts
  the four large-backlog exchange tests first so they overlap the shorter
  tests instead of extending the end of the run.
