...
| `api/` | shared command core between `cli::` and `serve::routes::`: `api::<cmd>::run(&mut Ctx, Request)` holds each subcommand's logic — a *move* of each `cli::<cmd>::run`'s middle (arg-parsing and TTY/`--json` rendering stay in `cli::`), so neither surface duplicates it (reuse precedent: `retrieval::code_search::search_code_hits`, generalized here to every command). Every `Request` derives `#[serde(deny_unknown_fields)]`, enforced by `tests/api__parity.rs`'s clap-introspection walk. `utilities::context::Ctx` bundles `Paths` + `Config` with a connection that is either `Borrowed` (the CLI's own connection, or the server's shared per-request one) or `Lazy` (opened on first `Ctx::conn()` call — a job worker's own dedicated connection; conn-free commands like `doctor`, `rebuild`, `ast`, `install-hooks`, `install`, `completions` never open one at all). One file per subcommand (`api::prune`, `api::rebuild`, `api::doctor`, …) plus the console-compat surface `api::stats`, plus two directory modules whose donor files sat near the 300-line ceiling: `doctor/` (+ `checks`, one function per health probe), `rebuild/` (+ `copy` and `documents`, the code-index/learning-state/document-domain ATTACH-copy run before the atomic DB swap, which itself snapshots the still-live DB to `comemory.db.pre-rebuild.bak` first). cwd-dependent middles (`save --ref-*` anchoring, the code rerank's working-set prior) resolve against the *calling process's* cwd — the server's cwd over HTTP, not the HTTP client's, documented behavior rather than a bug. The console-only cores (no CLI subcommand of their own, reached only through `serve::routes`) sit beside them: `overview`, `reembed`, `gc_policy`. The code-capability cores — `ast` (now `pattern_search`), `index_code`, `ingest_code`, `index_runs`, `repos`, `repo_admin`, `hooks`, `install_hooks` — moved to `domains/code/` with #167, the memory cores — `save`, `delete`, `list`, `show`, `update`, `restore`, `trash`, `refresh_refs` — to `domains/memories/` with #169, the graph cores — `graph` (now `view`), `graph_nodes`, `graph_recompute`, `edges` — to `domains/graph/` with #170, the retrieval cores — `search`, `search_code`, `context`, `find`, `suggest`, `config_retrieval` — to `domains/retrieval/` with #171, and the learning cores — `feedback`, `eval`, `mine`, `tune`, `bandit`, plus the console-only `learning` (now `console`) and `learning_proposals` — to `domains/learning/` with #173 |
...
- **D4 — `src.nested` replaces the starter map** with the exact, project-specific
  ...
  - `domains/*`: `ast`, `cloud`, `document`, `evaluation`, `exchange`, `index_code`, `memory_store`, `repos`, `source`, `unified`, `tests`, `proptest-regressions`
