...
//! Test mirror for `src/utilities/when.rs` — the `--since` / `--until` /
//! `--as-of` value parser.
//!
//! Building a `TimeScope` from three parsed instants moved to
//! `domains::retrieval::scope` with #178, so that half is mirrored by
//! `tests/domains__retrieval__scope.rs`.

use comemory::utilities::when::{DayEdge, parse_when};
