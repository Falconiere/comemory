...
fn env_code_threshold_override_applies() {
    ...
    std::env::set_var("COMEMORY_RETRIEVAL_CODE_THRESHOLD", "0.7");
    let result = Config::defaults().with_env();
...
fn env_code_threshold_invalid_is_an_error() {
    ...
    for bad in ["not-a-number", "1.5", "-0.1", "nan"] {
        std::env::set_var("COMEMORY_RETRIEVAL_CODE_THRESHOLD", bad);
        let result = Config::defaults().with_env();
        std::env::remove_var("COMEMORY_RETRIEVAL_CODE_THRESHOLD");
        let err = result.expect_err(&format!("'{bad}' must error"));
        let msg = err.to_string();
...
fn env_code_bm25_weights_triple_applies() {
    ...
    std::env::set_var("COMEMORY_RETRIEVAL_CODE_BM25_WEIGHTS", "3.0,1.0,2.0");
    let result = Config::defaults().with_env();
    std::env::remove_var("COMEMORY_RETRIEVAL_CODE_BM25_WEIGHTS");
...
fn env_code_bm25_weights_bad_shape_or_value_is_an_error() {
    ...
    for bad in ["2.0,1.0", "x,1,1", "-1,1,1", "0,0,0"] {
        std::env::set_var("COMEMORY_RETRIEVAL_CODE_BM25_WEIGHTS", bad);
        let result = Config::defaults().with_env();
        std::env::remove_var("COMEMORY_RETRIEVAL_CODE_BM25_WEIGHTS");
        let err = result.expect_err(&format!("'{bad}' must error"));
        let msg = err.to_string();
...
fn env_near_dup_hamming_override_applies() {
    ...
    std::env::set_var("COMEMORY_RANK_NEAR_DUP_HAMMING", "4");
    let result = Config::defaults().with_env();
    std::env::remove_var("COMEMORY_RANK_NEAR_DUP_HAMMING");
...
fn env_near_dup_hamming_over_64_is_an_error() {
    ...
    std::env::set_var("COMEMORY_RANK_NEAR_DUP_HAMMING", "65");
    let result = Config::defaults().with_env();
    std::env::remove_var("COMEMORY_RANK_NEAR_DUP_HAMMING");
...
fn env_superseded_grace_days_override_applies() {
    ...
    std::env::set_var("COMEMORY_PRUNE_SUPERSEDED_GRACE_DAYS", "14");
    let result = Config::defaults().with_env();
    std::env::remove_var("COMEMORY_PRUNE_SUPERSEDED_GRACE_DAYS");
...
fn env_superseded_grace_days_invalid_is_an_error() {
    ...
    std::env::set_var("COMEMORY_PRUNE_SUPERSEDED_GRACE_DAYS", "-1");
    let result = Config::defaults().with_env();
    std::env::remove_var("COMEMORY_PRUNE_SUPERSEDED_GRACE_DAYS");
...
fn tune_grids_have_no_env_overrides() {
    ...
    std::env::set_var("COMEMORY_TUNE_RRF_K_GRID", "20.0,60.0");
    let result = Config::defaults().with_env();
    std::env::remove_var("COMEMORY_TUNE_RRF_K_GRID");
