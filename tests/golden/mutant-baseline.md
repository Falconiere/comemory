...
Modules scoped: `src/output/**`, `src/memory/**`, `src/domains/graph/**`, `src/serve/**`.
...
- `src/output/graph.rs:56` — replace `>` with `>=` in `to_dot`
- `src/output/search.rs:63` — replace `==` with `!=` in `write_tty`
